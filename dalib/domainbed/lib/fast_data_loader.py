...
class FastDataLoader(object):
    ...
    def __init__(self, dataset, weights, batch_size, num_workers, length=EPOCH):
        super(FastDataLoader, self).__init__()

        if length == self.EPOCH and weights is not None:
            raise Exception(
                "Specifying sampling weights with len=EPOCH is "
